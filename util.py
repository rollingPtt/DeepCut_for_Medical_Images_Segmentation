...
def apply_seg_map(img, seg, alpha):
    ...
    """
    # np.savetxt('vector.txt', seg, fmt='%.6f')  # 这一行是我加的，用来查看 seg 的输出
...
##########################################################################################
# 从这后面的函数都是我添加的
def get_object_bbox_binary(seg):
    """
    从二值分割掩码中提取物体边界框
    @param seg: 这里的 seg 必须是二值分割掩码 (1=背景, 0=物体)
    @return: (x_min, y_min, x_max, y_max) 边界框坐标
    """
    # print(seg)
    binary_mask = (seg == 1).astype(np.uint8)
